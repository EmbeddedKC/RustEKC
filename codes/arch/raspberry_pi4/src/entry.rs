...
use crate::util::console::print_raw;
...
pub fn t_breakpoint() {
    unsafe{
        ...
        TCR_EL1.write(TCR_EL1::IPS::Bits_40);
        barrier::isb(barrier::SY);
        // Flush TLB
        crate::arch_flush_tlb(0);
        print_raw("breakpoint reached.");
        loop{}
    }
}
...
unsafe fn switch_to_el1() {
    let lr: u64 = _start as u64;
    //return address of this func, hardcode
...
unsafe fn init_boot_page_table() {
    //1000 200000 4000_0000

    // 0x0000_0000_0000 ~ 0x0080_0000_0000, table
    BOOT_PT_L0[0] = PageTableEntry::new_table(VirtAddr::new(BOOT_PT_L1.as_ptr() as usize).into());
    
    // 0x0000_0000_0000..0x0000_4000_0000, block
    BOOT_PT_L1[0] = PageTableEntry::new_page(
        PhysAddr::new(0),
        MapPermission::R | MapPermission::W | MapPermission::X,
        true,
    );
    // 0x0000_4000_0000..0x0000_8000_0000, block
    BOOT_PT_L1[1] = PageTableEntry::new_page(
        PhysAddr::new(0x4000_0000),
        MapPermission::R | MapPermission::W | MapPermission::X,
        true,
    );
    // 0x0000_8000_0000..0x0000_C000_0000, block
    BOOT_PT_L1[2] = PageTableEntry::new_page(
        PhysAddr::new(0x8000_0000),
        MapPermission::R | MapPermission::W | MapPermission::X | MapPermission::G,
        true,
    );

    // 0x0000_C000_0000..0x0001_0000_0000, block
    // containing FE00_0000, where uart and gpio locates.
    BOOT_PT_L1[3] = PageTableEntry::new_page(
        PhysAddr::new(0xC000_0000),
        MapPermission::R | MapPermission::W | MapPermission::X | MapPermission::G ,
        true,
    );
}
